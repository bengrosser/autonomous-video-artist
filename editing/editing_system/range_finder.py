...
def range_filter(range_result):
    """
    Make sure that there is no too short editing range
    :param range_result: Result generated by the range_finder
    :return: filtered range result
    """
    print "Before filter the range_result length is", len(range_result)
    filtered_ranges = []
    for editing_range in range_result:
        begin_frame = editing_range[0]
        end_frame = editing_range[1]
        range_length = end_frame - begin_frame
        if range_length > 48:
            filtered_ranges.append(editing_range)
    filtered_ranges = set(filtered_ranges)
    filtered_ranges = list(filtered_ranges)
    print "After filter the range_result length is", len(filtered_ranges)
