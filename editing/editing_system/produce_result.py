...
from frame_cluster import cluster_with_threshold, cluster_with_threshold_range
...
def generate_frame_gradient(frame):
    ...
    """
    color_dir_map = np.array(
        [[0, 190, 200], [200, 70, 70], [51, 204, 233], [120, 80, 220], [220, 0, 80], [200, 200, 60]])
...
# TODO: Write comment about the specific inserting mechanism
...
def generate_clip_cluster_key(vid_name, clip_range):
    """
    Helper function for generate_video_with_range
    Generate key for clustered_clip
    :param vid_name: The name of the video where the block comes from
    :param clip_range: The range specified in the editing block
    :return: the key for clustered clips
    """
    begin_frame = str(clip_range[0])
    end_frame = str(clip_range[1])
    result_name = vid_name + "_" + begin_frame + "_" + end_frame
    return result_name
...
def generate_video_with_range(assemble_blocks, output):
    ...
    for editing_block in assemble_blocks.editing_blocks:
        ...
        if clip_key not in clustered_clips:
            ...
            # clustered_video = adaptive_cluster(camera, editing_block.cluster_threshold)
            print "Spend", time.time() - cluster_start_time, "to cluster with threshold", block_threshold, "with range", block_range
            clustered_clips[clip_key] = clustered_video
            cluster_to_use = clustered_video[block_cluster_index]
            num_frames = len(cluster_to_use)
            frame_counter = 0
            for frame in cluster_to_use:
            ...
            cluster_to_use = clustered_clips[clip_key][block_cluster_index]
            num_frames = len(cluster_to_use)
            frame_counter = 0
            for frame in cluster_to_use:
                edited_result.write(frame)
...
def generate_video_with_visuals_with_range(assemble_blocks, output):
    """
    Generate result videos with visual bridges
    :param assemble_blocks: abstract data result from editing
    :param output: output name for the video
    ...
    for editing_block in assemble_blocks.editing_blocks:
        ...
        if clip_key not in clustered_clips:
            ...
            cluster_to_use = clustered_clips[block_vid_name][block_cluster_index]
            write_frames_with_visual(visual_result, assemble_blocks, cluster_to_use, counter)
        counter += 1
        print "Finished the block"
    print "Spend", time.time() - start_time, "to generate video"
